      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        ...
        <div className="flex flex-col gap-6 px-1 md:px-0">
          {METRICS.map((metric, index) => (
            <AnimationContainer key={`metric-${index}`} animation={metric.reverse ? "fadeLeft" : "fadeRight"} delay={0.6 + index * 0.2}>
              <div className="relative rounded-3xl bg-[#191919] p-4 lg:p-6 overflow-hidden z-0">
                <AnimationContainer animation="scaleUp" delay={0.7 + index * 0.2}>
                  <div className={cn("absolute -bottom-1/2 right-0 bg-primary size-20 lg:size-32 blur-[3rem] lg:blur-[5rem] rounded-full -z-10", metric.reverse && "left-0")} />
                </AnimationContainer>
                <div className={cn("flex items-center justify-between gap-6 z-30", metric.reverse && "flex-row-reverse")}>
                  <AnimationContainer animation="fadeUp" delay={0.8 + index * 0.2}>
                    <div className="flex flex-col">
                      <div className="flex items-baseline gap-1">
                        <span className="text-4xl font-medium">{metric.number}{metric.suffix ?? ""}</span>
                      </div>
                      <p className="text-sm text-muted-foreground">{metric.label}</p>
                    </div>
                  </AnimationContainer>
                  <AnimationContainer animation="fadeUp" delay={0.9 + index * 0.2}>
                    <Image src={metric.image} alt={metric.label} width={64} height={64} className="size-12 lg:size-16" />
                  </AnimationContainer>
                </div>
              </div>
            </AnimationContainer>

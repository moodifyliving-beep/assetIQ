...
export default function InvestDashboard() {
  ...
  const fetchApprovedProperties = async () => {
    try {
      ...
      const transformed = properties.slice(0, 3).map((property: any) => ({
        id: property.id,
  ...
  return (
    <DashboardLayout>
      <AlertDialog open={showBetaNotice} onOpenChange={setShowBetaNotice}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Moodify is currently in beta</AlertDialogTitle>
            <AlertDialogDescription>
              Trading and real investments are not yet enabled. All properties, portfolio data, and investment
              actions in this dashboard are for demonstration purposes only while we onboard partners.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogAction onClick={() => setShowBetaNotice(false)}>I understand</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <Link href="/property/add-property">
                <Button variant="outline" className="h-12 w-full">List New Asset</Button>
              </Link>
              <Link href="/invest/royalties">
                <Button variant="outline" className="h-12 w-full">Claim Royalties</Button>
              </Link>
            </div>
          </CardContent>
        </Card>
